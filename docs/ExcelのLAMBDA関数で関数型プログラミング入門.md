...
# Excelで「関数型脳」を作る Vol.2：関数は「値」だった —— 偶然のタイプミスとチャーチ数の衝撃
で、`x = x + 1` という式への違和感から、関数型プログラミングでは「箱(変数)の中身を入れ替える(代入)のではなく、値につける名札(定義)をすること」を学びました。
今回は **「関数も値である」** という話をします。かっこよく言えば「学習の結果」ですが、実際は、**ただのタイプミス**からの気づきでした。
...
* `_SUM = SUM` (`SUM` という関数に、`_SUM` という名前をつける)
...
> **「1」とは、ある関数 `f` を、1回適用することである。**
> **「2」とは、ある関数 `f` を、2回適用することである。**
...
次回、Vol.3では、「関数を生産する工場 —— カリー化」の話をします。
...
* `SUM` を変数に入れるタイプミスが、関数と値が一緒に扱えるという体験が目から鱗でした。
* 関数型プログラミングってメンドクサイことをやっていて、難解だなぁと思うのですが、打てば響くというか、関数をつつくと何か返ってくるという単純な仕組みの積み重ねで、全てをやろうとして、それができてしまうことが、実はすごいことなんじゃないのか？と思いました。
* `1` や `2` は単なるデータではなく、「回数」という振る舞いを持った関数とみなせる。
...
---
# Excelで「関数型脳」を作る Vol.4：配列の壁を越えろ —— 「遅延評価（Thunk）」と高階関数の罠

前回（Vol.3）、私は「引数を固定した専用関数」を量産する工場を作りました。
道具は完璧です。あとは、この関数を使って、100行でも1,000行でも一気に一括処理するだけです。

「Excelの数式には `For` ループがない」。
そんな悩みを解決するために見つけたのが、**`MAP` 関数** でした。

`MAP` は、配列の要素を1つずつ取り出し、関数（LAMBDA）に渡して、結果を新しい配列として返してくれます。
...
「素晴らしい！ これでVBAのループから完全に卒業できる！」
そう歓喜したのも束の間、実務のデータを処理しようとして、私は**Excel数式最大のトラップ**に引っかかってしまいました。

---

## 1. 絶望の `#CALC!` エラー

実務でやりたいことは、単なる「1つの値から1つの値」への変換ではありません。
「1つの値（商品IDなど）から、複数の値（商品名と価格など）を横に並べて返したい」というケースがほとんどです。

そこで、受け取った数値 `x` と、その二乗 `x^2` を横に並べる（`HSTACK`）数式を書いてみました。
...
エンターキーを押した瞬間、セルに表示されたのは無情なエラーでした。
**`#CALC! （ネストされた配列はサポートされていません）`**

調べてみると、信じられない事実が判明しました。
**「MAP関数は、必ず1つのセル（スカラ値）を返さなければならない」**
配列の中に配列を入れること（ジャグ配列）を、Excelのエンジンは許してくれなかったのです。

「1列の結果しか返せないなら、実務のテーブル処理に使えないじゃないか……！」
...
完全に手詰まりだと思いました。
配列が返せない。複数列のデータが作れない。

その時、脳裏に Vol.2 での衝撃的な発見がフラッシュバックしました。
**「関数型言語では、関数も『値』として扱える」**

Excelは「1つの値（スカラ値）」しか返してはいけないと言っています。
数値の `10` はスカラ値です。文字列の `"A"` もスカラ値です。
そして……**「機能が詰まった関数（LAMBDA）そのもの」も、スカラ値ではないか？**

「配列（HSTACK）」を返そうとするから怒られる。
だったら、**「『配列を作りますよ』という手順書（関数）」**をポンと返してやれば、Excelは「あ、値が1つですね。どうぞ」と通してくれるはずです！

---

## 3. 「箱詰め職人」を作る（THUNKの概念）

このアイデアを実現するために、Vol.3で作った「関数を返す関数」のテクニックを使います。

数値を渡すと、**「引数なしで実行したら `HSTACK(x, x*x)` を返す関数」** を生成する工場を作ります。

```excel
// x を受け取ると、「() -> HSTACK(x, x*x) という関数」を返す
_箱詰め職人, LAMBDA(x, LAMBDA(HSTACK(x, x * x)))

```

内側の `LAMBDA()` には引数がありません。ただ「実行されるのを待っている箱」です。
プログラミング用語で、このような「後で計算するためのカプセル」を **Thunk（サンク）** と呼びます。

この職人を `MAP` に渡してみましょう。

```excel
_箱のリスト, MAP(SEQUENCE(5), _箱詰め職人)

```

エラーになりません！
Excelから見れば、`_箱のリスト` の中身は `{LAMBDA; LAMBDA; LAMBDA; LAMBDA; LAMBDA}` という5つの「値（関数オブジェクト）」です。
ネストされた配列ではなく、「関数のリスト」を作ることには成功しました。

---

## 4. 箱を開けて、積み上げる（REDUCEで回収）

あとは、この5つの箱（関数）を順番に「実行」して、中身の配列を取り出し、縦にガッチャンガッチャンと積み上げていくだけです。

ここで登場するのが、リストを順番に合体させていくための強力な関数 **`REDUCE`** です。（詳しい使い方はVol.5で解説します）

先ほどのアイデアをすべて繋げた、完全な数式がこれです！

```excel
=LET(
    // 1. Thunk（遅延実行カプセル）を作る工場
    _MakeThunk, LAMBDA(x, LAMBDA(HSTACK(x, x * x))),
    
    // 2. MAPで、計算結果の代わりに「関数のリスト」を生成する（エラー回避！）
    _thunks, MAP(SEQUENCE(5), _MakeThunk),
    
    // 3. REDUCEで箱を順番に開けて、縦に積み上げる
    REDUCE(
        {"x", "x^2"}, // 見出し（初期値）
        _thunks,      // 関数のリスト
        LAMBDA(acc, thunk, 
            VSTACK(acc, thunk()) // ここが魔法！ thunk() でカプセルを実行する
        )
    )
)
...
数式の最後にある **`thunk()`** に注目してください。
名前に `()` を付けるだけで、カプセル化されていた「配列を作る計算」がその場でパカッと開き、`VSTACK` で綺麗に合体していくのです。

結果は見事に、見出し付きの2列×5行のテーブルとしてスピルしました！

---

## 5. まとめ：型破りな「関数型脳」

今回は、Excelエンジンの「配列をネストできない」という絶対的な制限をハックしました。

* **課題:** `MAP` は配列を返せない（スカラ値のみ）。
* **解決策:** 配列を返すのではなく、**「配列を返す関数（Thunk）」**を値として返す。
* **実行:** 後から `thunk()` と空のカッコをつけて実行（遅延評価）し、結合する。

「関数は値である」というVol.2の知識と、「関数を返す関数」というVol.3の知識が組み合わさった時、それは単なる理屈ではなく、**システムの限界を突破するための最強の武器**になりました。

「計算を後回しにする（遅延評価）」。
これもまた、関数型プログラミングの美しく、そして強力な特徴の一つです。

さて、今回のハックの最後で、箱を開けて積み上げるために **`REDUCE`** という関数を使いました。
実はこの `REDUCE` こそが、データ処理の最終形態であり、すべてのロジックを一つに束ねるラスボスです。

次回、**Vol.5「すべてを一つに —— 畳み込み（REDUCE）」**。
これさえマスターすれば、もうExcelの数式で表現できないことはありません。

---

### 今日の気づき

* `MAP` 関数は、配列（複数セル）を返すことができない。
* しかし、配列を返す **「引数なしの関数（LAMBDA）」** を返すことならできる。
* 関数を「実行待ちのカプセル（Thunk）」として持ち運び、後で `()` をつけて実行するテクニックは、複雑なデータ処理の特効薬になる。
