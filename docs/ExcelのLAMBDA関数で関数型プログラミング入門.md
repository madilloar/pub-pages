...
プログラミングを始めたばかりの頃、どうしても納得できない式がありました。
これです。
...
中学の数学（代数）で育った私の脳は、この式を見てこう叫びました。
**「いや、両辺から  を引いたら  になるじゃん！ 矛盾してるよ！」**

先輩からは「これは方程式じゃない。**代入（Assignment）**だ。 という箱の中身を書き換える命令なんだ」と教わりました。
「そういうものか」と飲み込んで数年。VBAで `i = i + 1` を書きまくっていた私は、Excelに搭載された **LAMBDA（ラムダ）関数** に出会い、再びあの頃の感覚を呼び覚ますことになります。

**「あ、やっぱり  っておかしかったんだ」**

連載第1回のテーマは、関数型プログラミングの根幹をなす**「名前（Name）」と「不変性（Immutability）」**についてです。

---

## 1. 「箱」に入れるか、「名札」を付けるか

なぜ、これまでのプログラミング（命令型）では `x = x + 1` が許されたのでしょうか？
それは、変数を**「値を出し入れできる箱」**として扱っていたからです。

* **命令型の世界（VBA）：**
「 という箱がある。今、中身は10だ。そこから10を取り出し、1を足して11にし、もう一度箱に戻せ（再代入）」
ここでは、時間は経過し、 の中身は**変化（Mutation）**します。

一方、関数型プログラミング（そして数学）のアプローチは全く異なります。
変数は「箱」ではありません。**「値につけられた名札（ラベル）」**です。

* **関数型の世界（LAMBDA）：**
「この世界に `10` という値がある。これに  という**名前を定義（Define）**する。
もし `11` が必要なら、 を書き換えるのではない。`11` という新しい値を用意し、それに  という新しい名前を付ける」

一度「 は 10 である」と**定義（束縛：Bind）**したら、その世界の中で  が 11 に変わることは絶対にありません。
これを**「不変性（Immutability）」**と呼びます。

---

## 2. Excelは「定義」の集まりだった

この「一度決まったら変わらない」という感覚、どこかで味わったことがあると思ったら……これ、**Excelのシートそのもの**なんですよね。

セルA1に `10` と入力し、B1に `=A1+1` と入力する。
B1の結果は `11` です。
このとき、A1の値が勝手に書き換わったりしません。A1はA1のまま、B1は新しい値として存在する。

Excelを使っている時、私たちはセルに対して「書き変われ！」と命令しているわけではありません。
**「A1とは10のことである」「B1とはA1に1を足したものである」** と、事実を**定義**しているだけなのです。

VBA（命令型）で無理やり「セルの値を上書き更新」する処理を書くと、元に戻せなくて冷や汗をかくことがあります。
でも、数式なら「元の定義（A1）」は守られたまま、「新しい定義（B1）」が生成されるだけ。

この**「不変性」**こそが、数式が壊れにくく、安心して使える理由だったんだと腹落ちしました。

---

## 3. 副作用のない世界は、こんなに平和だ

「名前の中身が書き換わらない」と何がいいのか？
一番の発見は、**「コードを読んだ時の安心感が段違い」**だということです。

VBAのコードを追うときは、常に疑心暗鬼でした。
「この `Count` 変数、この行に来るまでにどこかで書き換えられてないか？ 条件分岐の中でイタズラされてないか？」

でも、LAMBDA（純粋関数）の世界は違います。
**「入力が同じなら、出力は絶対に同じになる」** というルールが徹底されています。

* **副作用（Side Effect）がない:**
計算の途中で「外部のセルの色を変える」とか「別の名前の中身を書き換える」といったことは絶対に起きません。

この制約があるおかげで、バグの原因を探すときも「その数式の中だけ」を見ればいい。このシンプルさに気づいた時、不便だと思っていた「不変性」が、最強の武器に見えてきました。
...
とはいえ、実務ではループ処理が必要です。
「変数を書き換えられない（再代入できない）のに、どうやって `i = 1` から `10` までカウントアップするんだ？」

ここでの発想の転換が、私にとって一番のブレイクスルーでした。
**「変数を変化させる」のではなく、「新しい値を生成する」**のです。

VBA脳だった私はこう考えていました。

* 「`i` という箱を用意して、1を入れて、次に2に入れ替えて……」

関数型脳になった私はこう考えます。

* 「1から10までの配列（`SEQUENCE`）という**新しい値**をドンと生成する」
* 「その配列全体を一気に処理する」
...
ここには「時間経過とともに変化する `i`」はいません。
あるのは「最初から存在する1〜10のリスト」という**値**だけです。

---

## コラム：名前は「日本語」でいい。ユビキタス言語の実践

「プログラミングでは、変数名は英語にするべきだ」
そんな常識を、Excelにおいては一度忘れましょう。

私たちは今、業務ロジックを記述しています。
現場の担当者が「売上総利益」と呼んでいるものを、コードの中で `GrossMargin` と翻訳する必要はありません。翻訳は認知負荷（脳のメモリ消費）を高め、バグの温床になります。

**「現場で使われている言葉（ユビキタス言語）を、そのまま数式の中で使う」**
これが、メンテナンス可能なExcelシートを作るコツです。
...
半年後のあなたが見たら、「 って何？  って何？」と悩みます。

### 良い例：英語（プログラマ的）

```excel
=LET(price, A1, taxRate, 0.1, price * taxRate)

```

かっこいいですが、日本人が読むには一瞬の「脳内翻訳」が必要です。
...
どうですか？ 数式がそのまま「文章」として読めますよね。
Excelの数式エンジンは、日本語（Unicode）の名前定義を完璧にサポートしています。これを使わない手はありません。

### ルールは1つだけ：「アンダースコア」を付ける

日本語を使う際、一つだけ推奨するルールがあります。
...
1. **Excel標準機能と区別する:** 将来もしExcelに `TEIKA` 関数などが追加されても、`_TEIKA` なら衝突しません。
2. **入力支援（インテリセンス）:** 数式バーで `_` を入力すると、あなたが定義した日本語の名前だけがズラッと候補に表示されます。これが驚くほど快適です。

値に名前を付けるときは、**「同僚に通じる日本語」**で、**「`_` を付けて」**定義する。
これだけで、あなたの数式は「処理」から「業務マニュアル」へと進化します。

---

## 5. まとめ：代入をやめ、定義しよう

中学時代、「x=x+1」に感じたあの違和感。
あれは、私の直感が「関数型」だったからかもしれません。

プログラミングとは、命令して変数を書き換えまくること（代入）だと思っていました。
でも、Excel LAMBDAが教えてくれたのは、**「値に名前を付け（定義）、事実を積み上げていく」**プログラミングの楽しさでした。

* **命令型:** 箱の中身を入れ替える（代入）。
* **関数型:** 値に名札を付ける（定義）。

さて、「数値」や「文字列」が値であり、それに名前を付けられることは分かりました。
ここでふと、恐ろしい予感がします。

**「もし『関数そのもの』も値だとしたら、関数にも名前を付けられるんじゃないか？」**

次回、この疑問から私のExcel観がさらに崩壊することになります。
タイプミスから生まれた衝撃の発見、**Vol.2「関数は値だった」**へ続きます。
